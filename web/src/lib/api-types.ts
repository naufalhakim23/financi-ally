...
export interface paths {
    ...
    "/auth/refresh": {
         ...
         *
         *     `refresh_token` is optional: a browser client posts `{}` and the server
         *     reads the token from the httpOnly `fa_refresh` cookie instead
         *     (WEB_COOKIE_AUTH). Mobile keeps sending it in the body.
         */
    ...
    "/auth/logout": {
         ...
         * @description `refresh_token` is optional for the same reason as /auth/refresh — a browser posts `{}` and presents the token through the httpOnly `fa_refresh` cookie, which this call also clears.
         */
    ...
    "/accounts/{id}": {
        ...
        head?: never;
        /**
         * Rename and/or archive an account
         * @description Both fields are optional; omitting one leaves it unchanged, so rename, archive and restore are the same call. At least one must be present.
         */
    ...
    "/entries/{id}": {
        ...
        post?: never;
        /**
         * Remove a posted entry from the ledger
         * @description Soft-delete: the row survives for audit, but every read path (balances, reports, list, sync pull) filters it out, so the money is gone from the book immediately. Idempotent — deleting twice still answers 204.
         */
        delete: operations["deleteEntry"];
        options?: never;
        head?: never;
        /**
         * Relabel a posted entry
         * @description The memo is the only mutable field on a posted entry. Amounts, accounts and the date *are* the posting — correcting one means deleting the entry and posting a new one, which is what the clients do.
         */
...
export interface components {
    schemas: {
        ...
        RefreshRequest: {
            /** @description Omit (post `{}`) when the caller is a browser relying on the httpOnly fa_refresh cookie. Mobile always sends it. */
            refresh_token?: string;
        };
        ...
        /** @description Every field optional; an omitted field is left unchanged. Supplying neither is a 400 — an empty patch is a mistake, not a no-op. */
...
export interface operations {
    ...
    updateAccount: {
        ...
        responses: {
            ...
            /** @description malformed body, or neither field supplied */
            400: {
                headers: {
                    [name: string]: unknown;
                };
